...
extension ScannerViewController:AVCaptureMetadataOutputObjectsDelegate {
    func metadataOutput(_ captureOutput: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        if metadataObjects.count > 0 {
            ...
            print(string, "hhhhhhh")
            
            let controller = UIAlertController.init(title: "优惠券详情", message: string, preferredStyle: UIAlertControllerStyle.alert)
            let actionCancel = UIAlertAction(title: "取消", style: .cancel, handler: { (action) in
                self.session.startRunning()
            })
            let actinSure = UIAlertAction(title: "使用", style: .default, handler: { (action) in
                UpdateData.editUsed(uuid: string)
                self.navigationController?.popViewController(animated: true)
            })
            controller.addAction(actionCancel)
            controller.addAction(actinSure)
